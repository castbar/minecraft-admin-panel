#!/bin/bash
# Test whitelist remove ejecutado dentro del contenedor

set -e

echo "============================================================"
echo "TEST: Whitelist Remove y Limpieza de Usuarios Test"
echo "============================================================"
echo ""
...
# Obtener o crear usuario admin y asegurar que tenga acceso a servidores
admin_user, created = User.objects.get_or_create(
    username='test_admin_whitelist',
    defaults={'is_staff': True, 'is_superuser': True}
)
admin_user.set_password('test_password_123')
admin_user.save()

# Asegurar que el usuario tenga acceso a al menos un servidor
...
# Obtener servidor cobblemon (el que está funcionando)
...
# Buscar servidor cobblemon directamente en la BD
...
if status_response.status_code == 200:
    ...
    if status_data.get('success'):
        ...
        if players_online:
            player_names = ', '.join(players_online[:10])
            print(f'   Usuarios: {player_names}')
else:
...
if whitelist_users:
    user_names = ', '.join(whitelist_users[:10])
...
import time
test_username = f'TestRemove_{int(time.time())}'
...
if test_users:
    ...
    for user in test_users:
        ...
        if remove_response.status_code == 200:
            ...
            error_msg = remove_response.content.decode()[:100]
            print(f'      ⚠️  Status {remove_response.status_code}: {error_msg}')
